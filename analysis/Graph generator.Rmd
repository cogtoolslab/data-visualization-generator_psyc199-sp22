...
graph_saver <- function(df, vars, graph_type, dfname, savepath){ #prefix
  ...
  if( graph_type == "histogram" ){
    # histograms for 4 numerical variables
    plot <- ggplot(df) +
      geom_histogram(aes(x = get(vars), y = stat(count) / sum(count) *100), position = "identity", alpha = 1, bins = 15) +
      labs(x=vars, y="Frequency (%)")
  }
  ...
  if( graph_type == "bar" ){
    # Bar graph for categorical variables 
    plot <- ggplot(df) + 
      geom_bar(aes(x = get(vars), y = stat(count) / sum(count) *100), position = "identity", alpha = 1) +
      labs(x=vars, y="Frequency (%)")
  }
...
Questions_generator <- function(fp) {
  ...
  # determine variable types from df.csv
  df_input <- read_csvy(fp, stringsAsFactors = T)
  ...
  X1_type <- ifelse(df_graph_X1_X2$X1 %in% varnames_N, "N", ifelse(df_X1_X2$X1 %in% varnames_C, "C", NA))
  X2_type <- ifelse(df_graph_X1_X2$X2 %in% varnames_N, "N", ifelse(df_X1_X2$X2 %in% varnames_C, "C", NA))
  
  df_X1_X2_types <- data.frame(X1=df_graph_X1_X2$X1, X2=df_graph_X1_X2$X2, X1_type=X1_type, X2_type=X2_type)
  ...
  df_sub <- df_X1_X2_types[(df_X1_X2_types$X1_type == "N") & (is.na(df_X1_X2_types$X2_type)), ]
  
  df_sub <- subset(df_sub, select=c("X1","X2"))
  ...
  df_sub <- df_X1_X2_types[(df_X1_X2_types$X1_type == "C") & (is.na(df_X1_X2_types$X2_type)),]
  df_sub <- subset(df_sub, select=c("X1","X2"))
  df_sub <- df_sub[!duplicated(df_sub), ]
  
  list_levels <- sapply(df_input[, unique(df_sub$X1)], levels)
  ...
  df_sub <- merge(df_sub, df_level, by=c("X1"), all.x=TRUE)
  ...
  Q_C_NA <- c(
    ...
    'sprintf("Which one has the larger proportion in %s, %s or %s?", df_sub_Q$X1, df_sub_Q$l1, df_sub_Q$l2)',
    # X (Level): Compare smaller
    'sprintf("Which one has the smaller proportion in %s, %s or %s?", df_sub_Q$X1, df_sub_Q$l1, df_sub_Q$l2)',
  
    # X (Count): Compare larger
    'sprintf("How many groups in %s has the larger proportion than %s?", df_sub_Q$X1, df_sub_Q$level)',
    # X (Count): Compare smaller
    'sprintf("How many groups in %s has the smaller proportion than %s?", df_sub_Q$X1, df_sub_Q$level)',
  ...
  df_Q_C_NA <- subset(df_Q_C_NA, select=c("X1", "X2", "Questions"))
  ...
  # varnames_N -------------------------------------------------
  # "scatter"
  # 'sprintf("What is the range of", df_sub_Q$X1), "?")
  # 'sprintf("What is the standard deviation of", varnames_N), "?")
  # 'sprintf("What is the mean of", varnames_N), "?")
  # 'sprintf("What is the median of", varnames_N), "?")
  # 'sprintf("What is the maximum of", varnames_N), "?")
  # 'sprintf("What is the minimum of", varnames_N), "?")
  
  df_sub <- na.omit(df_X1_X2_types[(df_X1_X2_types$X1_type == "N") & (df_X1_X2_types$X2_type == "N") , ])
  df_sub <- subset(df_sub, select=c("X1","X2"))
  df_sub <- df_sub[!duplicated(df_sub), ]
  ...
  Q_N_N <- c(
    'sprintf("What is best prediction of %s if %s is %d?", df_sub_Q$X2, df_sub_Q$X1, df_sub_Q$value)',
    'sprintf("What is the relationship between %s and %s?", df_sub_Q$X1, df_sub_Q$X2)',
    'sprintf("There is no relationship between %s and %s.", df_sub_Q$X1, df_sub_Q$X2)'
  )
  ...
  df_Q_N_N <- subset(df_Q_N_N, select=c("X1", "X2", "Questions"))
  ...
  df_sub <- na.omit(df_X1_X2_types[(df_X1_X2_types$X1_type == "C") & (df_X1_X2_types$X2_type == "N") , ])
  df_sub <- subset(df_sub, select=c("X1","X2"))
  df_sub <- df_sub[!duplicated(df_sub), ]
  ...
  df_Q_C_N <- subset(df_Q_C_N, select=c("X1", "X2", "Questions"))
